...
- _2026-05-28_: **Test debt resolved.** Comprehensive test suite added across all packages: scanners (gomod, npm, python, dockerfile, compose, terraform, ansible, githubactions, elixir), vuln client/matcher (httptest mocks), policy engine, hygiene checks, report formatters (table, JSON, SARIF), CRA compliance engine, and supply chain analysis. All tests pass with `-race`.
- _2026-05-28_: **`context.Context` propagated.** All I/O-bound functions now accept `context.Context` as first parameter. Scanner interface, OSV client (uses `NewRequestWithContext`), matcher, policy loader, all report writers, CRA engine, analysis reporters. CLI commands use `cmd.Context()` from Cobra.
- _2026-05-28_: **All 19 v2 proposals archived.** Everything in `openspec/changes/` has been implemented and moved to `openspec/archive/`. No active proposals remain.
- _2026-05-28_: v0.3.0 features: `init-security` (SECURITY.md, security.txt, CRA config scaffolding), `init-ci` (GitHub Actions workflow), `diff` (scan comparison), Elixir scanner, licence detection (npm/PyPI registry APIs), Go vuln DB integration, enhanced policy engine (CRA scores, pinning thresholds, licence allow/deny), SARIF enrichment (helpUri, fingerprints, remediation).
- _2026-05-28_: **OSV client `baseURL` field added** for testability. `GoVulnClient` also has configurable `baseURL`. Both default to production URLs but can be overridden in tests via httptest.
- _2026-05-28_: `.gitignore` fix: `chainsaw` pattern was matching `cmd/chainsaw/` directory. Changed to `/chainsaw` to only match the binary at repo root.
